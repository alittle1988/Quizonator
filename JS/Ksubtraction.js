...
function moveToNext() {
    ...
    if(i >= 9){
        ...
        wA2.textContent =  Number(a1.textContent) + 1;
...
function createArray() {
    ...
    //creats qArray and answerArray
    for(let i = 0; i < 10; i++) {
        let thisNum = Math.floor(Math.random() * testArray.length)
        let thatNum = Math.floor(Math.random() * testArray2.length)
        if(thatNum > thisNum) {
            while(thatNum > thisNum) {
